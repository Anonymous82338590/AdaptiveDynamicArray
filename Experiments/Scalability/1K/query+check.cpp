...
int main() {
    ...
    string filepath[3] = {"querycheck.csv", "querycheckLog.txt"};
    ...
    for (int lt = 0; lt < TotalActions; lt++) {
        ...
        if ( (lt+1 <= 10) || ((lt+1<=100) && ((lt+1)%10 == 0)) || ((lt+1<=1000) && ((lt+1)%100 == 0))
            ...
            int * Anstv = tiered.RangeQuery(1, NowTotalNum, tmpnum);
            if (! CompareArray(Ansda, Anstv, tmpnum)) {
